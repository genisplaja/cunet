...
western = False
if western:
    fp = '/mnt/md1/genis/FTANet-melodic/eval_datasets/MDB-melody-synth/'
    western_wav = glob.glob(fp + 'audio_mix/*.wav')
    training_files = random.sample(western_wav, int(len(western_wav)*0.85))
    validation_files = [x for x in western_wav if x not in training_files]
    checkpoint_best_OA = '/mnt/sda1/genis/FTANet-melodic/model/western/OA'
else:
    fp = '/mnt/md1/genis/carnatic_melody_synthesis/resources/Saraga-Synth-Dataset/experiments_4/'
    checkpoint_best_OA = '/mnt/md1/genis/FTANet-melodic/model/old'
    ...
    for artist in tracks_to_train:
        files_for_artists = random.sample(artist, 70)
